...
class HSIDataLoader(object):
    ...
    def load_raw_data(self):  # 数据分类
        ...
        data_path = r'D:\python\SQSFormer-RWKV\data\Indian\%s_split.mat' % self.data_file#绝对路径
        print(data_path)
    ...
    def split_data_randomly(self, TR, TE, test_size=0.01):  #随机划分数据集
        # 只对 TR 进行随机划分，保留 TE 不变
        indices = np.argwhere(TR+ TE > 0)  # 只对 TR 进行操作
        train_indices, val_indices = train_test_split(indices, test_size=test_size, random_state=42)

        # 新的训练集和验证集
        TR_new = np.zeros_like(TR)
        VAL = np.zeros_like(TE)  # 验证集

        for idx in train_indices:
            TR_new[idx[0], idx[1]] = TR[idx[0], idx[1]]

        for idx in val_indices:
            VAL[idx[0], idx[1]] = TR[idx[0], idx[1]]

        return TR_new, TE  # 返回新的训练集和原始测试集
    ...
    def prepare_data(self):
        ...
        print(f"Initial training set size (before split): {np.sum(self.TR > 0)}")
        print(f"Initial test set size (before split): {np.sum(self.TE > 0)}")
